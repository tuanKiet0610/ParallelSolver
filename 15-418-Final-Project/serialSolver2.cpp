...
int BOARD_SIZE;   // length of board
MATRIX solution;
...
MATRIX initInput(char *filename) {
    ...
    // allocate memory for matrix with BOARD_SIZE x BOARD_SIZE dimensions
    matrix.data = (short**) malloc(BOARD_SIZE * sizeof(short*));  
    for (int i = 0; i < BOARD_SIZE; i++)
        matrix.data[i] = (short*) malloc(BOARD_SIZE * sizeof(short));

    matrix.fixed = (short**) malloc(BOARD_SIZE * sizeof(short*));
    for (int i = 0; i < BOARD_SIZE; i++)
        matrix.fixed[i] = (short*) malloc(BOARD_SIZE * sizeof(short));
...
short permissible(MATRIX matrix, short r, short c) {
    ...
    // check same column
    for (short line = 0; line < BOARD_SIZE; line++) {
...
void increasePosition(MATRIX *matrix, short *rPointer, short *cPointer) {
    ...
    } while (*rPointer < BOARD_SIZE && matrix->fixed[*rPointer][*cPointer] == 1);
}
...
item* createItem(MATRIX matrix, short i, short j) {
    ...
    node->mat.data = (short**)malloc(BOARD_SIZE * sizeof(short*));
    for (int m = 0; m < BOARD_SIZE; m++)
        node->mat.data[m] = (short*) malloc(BOARD_SIZE * sizeof(short));
    
    node->mat.fixed = (short**)malloc(BOARD_SIZE * sizeof(short*));
    for (int m = 0; m < BOARD_SIZE; m++)
        node->mat.fixed[m] = (short*)malloc(BOARD_SIZE * sizeof(short));
...
// initialize job pool with BOARD_SIZE jobs, taken from input board
void initPool(MATRIX *matrix){
    ...
    while(1) {
        ...
        if (matrix->data[i][j] <= BOARD_SIZE && permissible(*matrix, i, j) == 1) {
        ...
        } else if(matrix->data[i][j] > BOARD_SIZE) {
            if (current != NULL)
...
short processPool(MATRIX matrix) {
    ...
    while (current != NULL && found == 0) {
        ...
        while (level > 0 && i < BOARD_SIZE && found == 0) {
            if (currMat.data[i][j] < BOARD_SIZE) {    
